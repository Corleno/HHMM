...
def Initialization(test=None, name=None, min_age=None, dataset=None, age_inv=None, model=None, Z_prior=None):
    ...
    ages = pickle.load(open(data_location + "mcmcPatientAges", 'rb'))
    ...
    n_test = len(ages)
    testTypes_test = testTypes
    observations_test = observations
    ages_test = ages
    treatment_indx_test = treatment_indx
    censor_ages_test = censor_ages
    death_state_test = death_states
...
def Compute_pos_Z_test(p, save_res=False, verbose=False, dataset=None):  ### given no state Z | -
    global Z_pos
    # It is a Bernouli(p)
    ...
    if save_res:
        with open("model_res/p_post.pickle", "wb") as res:
...
def main(dataset=None):
    ...
    ####### Initialization ##########
    #################################
    Initialization(test=args.test, name=args.name, min_age=args.min_age, dataset=dataset,
    ...
    Compute_pos_Z_test(p, verbose=False, save_res=True, dataset=dataset)
    # Z_pos = Load_pos_Z_test()

    import matplotlib.pyplot as plt
    plt.hist(Z_pos, bins=20)
    plt.show()
...
if __name__ == "__main__":
    ...
    main(dataset=args.dataset_test)
