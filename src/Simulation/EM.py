...
if __name__ == "__main__":
    ...
    with open("synthetic_data2.pickle", "rb") as res:
        time, observations, _ = pickle.load(res)
    true_frailty = np.concatenate([np.ones(300), np.zeros(200)])
    ...
    # alpha1 = np.ones(4)
    # true
    # alpha0 = np.log(np.array([0.1, 0.1, 1., 1.]))
    # alpha1 = np.array([1., 1., 0.1, 0.1])
    # informative
    beta0 = -1
    beta1 = 1
    # noninformative
    ...
    # true
    # beta0 = np.log(1./0.95-1)
    # beta1 = np.log(1./0.05-1)
    ...
    for rep in range(n_replication):
        for t in range(T):
            ...
            print(confusion_matrix(true_frailty, (pos_z[:,0] > 0.5).astype(int)))
            print(accuracy_score(true_frailty, (pos_z[:,0] > 0.5).astype(int)))
        ...
        # import pdb
        # pdb.set_trace()
        with open("res/res_EM_full.pickle", "wb") as res:
