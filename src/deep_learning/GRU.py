...
class Model(object):
    ...
    def build_model(self):
        ...
        self.model.add(GRU(self.hidden_size,  return_sequences=False))
...
if __name__ == "__main__":
    ...
    GRU_model.fit(pad_features_train, y_train, n_epoch=200, n_iter=5)
    GRU_model.save_model()
    print("training time: {}s".format(time.time()-ts))

    ts = time.time()
    GRU_model.load_model()
    GRU_model.evaluate(pad_features_test, y_test, confusion=True)
