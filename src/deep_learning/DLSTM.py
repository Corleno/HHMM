...
# DLSTM to predict the last visiting statue
...
class Model(object):
    ...
    def save_model(self):
        ...
        with open("model_dlstm.json", "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        self.model.save_weights("model_dlstm.h5")
        print("Saved model to disk")

    def load_model(self):
        from keras.models import model_from_json
        # load json and create model
        json_file = open('model_dlstm.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        loaded_model = model_from_json(loaded_model_json)
        # load weights into new model
        loaded_model.load_weights("model_dlstm.h5")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--size', help="layer size", type=int, default=32)
    ...
    ts = time.time()
    DLSTM_model = Model()
    DLSTM_model.build_model()
    DLSTM_model.fit(pad_features_train, y_train, n_epoch=200, n_iter=5)
    DLSTM_model.save_model()
    print("training time: {}s".format(time.time()-ts))

    ts = time.time()
    DLSTM_model.load_model()
    DLSTM_model.evaluate(pad_features_test, y_test, confusion=True)
    print("testing time: {}s".format(time.time()-ts))
    # DLSTM_model.predict(pad_features_test)