...
if __name__ == "__main__":
    ...
    DLSTM_model.fit(pad_features_train, pad_output_train, n_epoch=200, n_iter=5)
    DLSTM_model.save_model()
    print("training time: {}s".format(time.time() - ts))

    ts = time.time()
    DLSTM_model.load_model()
    DLSTM_model.evaluate(pad_features_test, output_test, confusion=True)
